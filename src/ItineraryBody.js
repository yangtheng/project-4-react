...
class ItineraryBody extends Component {
  ...
  render () {
    return (
      <Panel className="col-sm-6" style={{backgroundImage: 'url(' + this.state.itinerary.bannerUrl + ')', backgroundSize: 'cover', height: '30vh', position: 'relative', width: '50%', float: 'right'}}>
        <h3>Title: {this.state.itinerary.title}</h3>
        <h3>Country: {this.state.itinerary.country}</h3>
        <h3>Days: {this.state.itinerary.days}</h3>
        <small>BannerUrl: {this.state.itinerary.bannerUrl}</small>
